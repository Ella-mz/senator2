...
@section('urlHeader') کسب و کار ها
@endsection
@section('header')
    کسب و کار ها
@endsection
...
@section('content')
    <section class="content">
        ...
        <div class="card">
            <form action="{{route('users.shops.filter.admin')}}" method="post">
                @csrf
                ...
                <div class="p-2">
                    <div class="row">
                        <div class="col-md-12 text-left">
                            ...
                            <a href="{{route('users.shops.index.admin', $active)}}" class="btn btn-secondary btn-sm">برگشت به حالت
                                اولیه</a>
                        </div>
                    </div>
                </div>
...
{{--                   class="btn btn-sm btn-primary" style="float: left">ایجاد محله جدید</a>--}}
                <h1 class="card-title" style="float: right">کسب و کار ها</h1>
            </div>
                        ...
                        <tr>
                            ...
                            @if(isset($shop->shop_logo))
                                <td width="80" height="40">
                                    <img src="{{asset($shop->shop_logo)}}" width="80" height="40">
                                </td>
                            @else
                                <td width="80" height="40">
...
{{--                                {{$shop->union->title}}--}}
                            ...
                            <td>
                                @if(isset($shop->shop_city_id))

                                {{$shop->city->title}}
                                @if(isset($shop->shop_neighborhood_id))
                            ...
                            <td>
                                @if($shop->shop_active=='active')
                                     <span style="color: #00B74A">تایید</span>
                                    @elseif($shop->shop_active=='disConfirm')
                                    <span style="color: #F93154">عدم تایید</span>
                                    @elseif($shop->shop_active=='inactive')
                                    غیرفعال
                                    @endif
                            ...
                            <td>
                                <a class="btn btn-primary btn-sm"
                                   href="{{ route('users.shops.detail.admin', $shop->id)}}">
...
{{--                                    @csrf--}}
                                    <a class="btn btn-success btn-sm"
                                       href="{{ route('users.shops.confirm.admin', $shop->id)}}">
                                        <i class="fa fa-check"></i>
                                    </a>
                                    <a data-toggle="modal" data-target="#disConfirmShop" data-id="{{$shop->id}}"
                                       data-reason="{{$shop->shop_reasonOfDeactivation}}"
...
{{--                                            onclick="return confirm('آیا از حذف فروشگاه {{$shop->shop_title}} اطمینان دارید؟')">--}}
