...
@section('title_realestate')لیست درخواست های ویژه کسب و کار شما
@endsection
@section('card_title')لیست درخواست های ویژه کسب و کار شما
@endsection
@section('css')
                    ...
                    <div class="col-lg-3 col-md-4 col-xs-12 d-lg-block d-none">
                        <div class="sidebar-wrapper search-sidebar">
                                                ...
                                                <li>
                                                    <div class="type-category accordion-item">
                                                        ...
                                                        @if($cat1->categories->count()>0)
                                                            <button type="button" class="catalog-link main accordion-button  "
                                                    ...
                                                    @if($cat1->categories->count()>0)
                                                        <ul class="show-more accordion-collapse collapse show"
                                                            id="panelsStayOpen-collapseOne"
                                                            aria-labelledby="panelsStayOpen-headingOne">
                                                            @foreach($cat1->categories()->where('active', 1)->orderBy('order', 'asc') as $cat_sub_1)
                                                                <li class="catalog-cat-item ">
                                                                    <div class="type-category accordion-item">
                                                                        <form action="{{route('applications.index.postedToSpecificAgency.panel', $adminOfAgency->id)}}"
                                                                              method="get"
                                                                    ...
                                                                    @if($cat_sub_1->categories->count()>0)
                                                                        <ul class="catalog-list inner-div accordion-collapse collapse show"
                                                                            ...
                                                                            @foreach($cat_sub_1->categories()->where('active', 1)->orderBy('order', 'asc')->get() as $cat_sub_2)
                                                                                <li>
                                                                                    <form
                                                                                        action="{{route('applications.index.postedToSpecificAgency.panel', $adminOfAgency->id)}}"
                                                                                        method="get"
                            ...
                            <form class="form-horizontal"
                                  ...
                                  {{--                                  action="{{route('applications.filter.realestate')}}"--}}
                                  method="post">
                                @csrf
                                @if($category1)
                                ...
                                @endif
                                {{--                                <button type="submit">l[pl[pkp</button>--}}
                                                                ...
                                                                <div class="contact-call-info">
                                                                    ...
                                                                    <div class="collapse multi-collapse"
                                                                        ...
                                                                        <div class="card card-body"
                                                                             id="contact-info{{$application->id}}"
                                                                        >
...
@section('js_realestate')
    <script>
        function contact(applicationId) {
            $('#contact-info'+applicationId).empty();
            jQuery.ajax({
                ...
                success: function (data) {
                    $('#contact-info'+applicationId).append(data.content)
                }
            });
        }
    </script>
    ...
    <script>
        $(document).ready(function () {
            $('#attributes_form_filter_page').on('submit', function (event) {
                ...
                $.ajax({
                    url: "{{route('applications.filter.postedToSpecificAgency.panel', $adminOfAgency->id)}}",
                    method: "POST",
    ...
    <script>
        $(document).ready(function () {
            $('#attributes_form_modal').on('submit', function (event) {
                event.preventDefault();
                $.ajax({
                    url: "{{route('applications.filter.realestate')}}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: 'json',
                    contentType: false,
