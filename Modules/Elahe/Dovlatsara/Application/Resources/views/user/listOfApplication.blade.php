                                        ...
                                        @foreach($categories as $key=>$cat1)
                                            <li>
                                                <div class="type-category accordion-item">
                                                    <form action="{{route('application.list.user')}}" method="get"
                                                          id="level1{{$cat1->id}}">
                                                        <input hidden name="category"
                                                               value="{{$cat1->id}}">
                                                        <a onclick="document.getElementById('level1{{$cat1->id}}').submit()"
                                                           style="cursor: pointer"> <span style="margin-right: 10px">{{$cat1->title}}</span></a>
                                                    </form>
                                                    @if($cat1->categories->count()>0)
                                                        <button type="button" class="catalog-link main accordion-button collapsed"
                                                ...
                                                @if($cat1->categories->count()>0)
                                                    <ul class="show-more accordion-collapse collapse"
                                                        ...
                                                        @foreach($cat1->categories()->where('active', 1)->orderBy('order', 'asc')->get() as $cat_sub_1)
                                                            <li class="catalog-cat-item ">
                                                                <div class="type-category accordion-item">
                                                                    <form action="{{route('application.list.user')}}"
                                                                          method="get"
                                                                ...
                                                                @if($cat_sub_1->categories->count()>0)
                                                                    <ul class="catalog-list inner-div accordion-collapse collapse"
                                                                        ...
                                                                        @foreach($cat_sub_1->categories()->where('active', 1)->orderBy('order', 'asc')->get() as $cat_sub_2)
                                                                            <li>
                                                                                <form
                                                                                    action="{{route('application.list.user')}}"
                                                                                    method="get"
...
@section('js_user')
    <script>
        function contact(applicationId) {
            ...
            jQuery.ajax({
                url: "{{route('applications.contact.user')}}",
                data: {
    ...
    <script>
        function setSeen(applicationId) {
            jQuery.ajax({
                url: "{{route('applications.recentSeen.user')}}",
                data: {
                    'applicationId': applicationId
                },
                type: "GET",
    ...
    <script>
        $(document).ready(function () {
            $('#attributes_form_filter_page').on('submit', function (event) {
                ...
                $.ajax({
                    url: "{{route('applications.filter.user')}}",
                    method: "POST",
